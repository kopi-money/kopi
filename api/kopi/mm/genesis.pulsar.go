...
func (x *fastReflection_GenesisState) Range(f func(protoreflect.FieldDescriptor, protoreflect.Value) bool) {
	...
	if x.NextLoanIndex != uint64(0) {
		value := protoreflect.ValueOfUint64(x.NextLoanIndex)
...
func (x *fastReflection_GenesisState) ProtoMethods() *protoiface.Methods {
	size := func(input protoiface.SizeInput) protoiface.SizeOutput {
		...
		if x.NextLoanIndex != 0 {
			n += 1 + runtime.Sov(uint64(x.NextLoanIndex))
		}
	...
	marshal := func(input protoiface.MarshalInput) (protoiface.MarshalOutput, error) {
		...
		if x.NextLoanIndex != 0 {
			i = runtime.EncodeVarint(dAtA, i, uint64(x.NextLoanIndex))
			i--
			dAtA[i] = 0x28
		}
		if len(x.DenomRedemptions) > 0 {
		...
		for iNdEx < l {
			...
			case 5:
				...
				for shift := uint(0); ; shift += 7 {
					...
					b := dAtA[iNdEx]
					iNdEx++
					x.NextLoanIndex |= uint64(b&0x7F) << shift
					if b < 0x80 {
						break
					}
				}
...
func (x *GenesisState) GetNextLoanIndex() uint64 {
	if x != nil {
