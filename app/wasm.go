...
func (app *App) registerWasmModules(
	...
	if err := app.RegisterStores(
		...
		return nil, fmt.Errorf("register stores: %w", err)
	}
	...
	if err = app.setAnteHandler(app.txConfig, wasmConfig, app.GetKey(wasmtypes.StoreKey)); err != nil {
		return nil, fmt.Errorf("setting ante handler: %w", err)
	}
	...
	if err = app.setPostHandler(); err != nil {
		return nil, fmt.Errorf("setting post handler: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("proto registry: %w", err)
	}

	if err = msgservice.ValidateProtoAnnotations(protoFiles); err != nil {
		return nil, fmt.Errorf("proto validator annotations: %w", err)
	}
...
func (app *App) setPostHandler() error {
	...
	if err != nil {
		return fmt.Errorf("new post handler: %w", err)
	}
