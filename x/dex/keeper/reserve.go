...
func (k Keeper) BeginBlockCheckReserve(ctx context.Context, eventManager sdk.EventManagerI, blockHeight int64) error {
	...
	return k.CheckReserve(ctx, eventManager)
	//}
...
// currencies.
func (k Keeper) CheckReserve(ctx context.Context, eventManager sdk.EventManagerI) error {
	...
	if found {
		if err := k.checkReserveForDenom(ctx, eventManager, address, base); err != nil {
	...
	for _, coin := range coins {
		...
		if err := k.checkReserveForDenom(ctx, eventManager, address, coin); err != nil {
